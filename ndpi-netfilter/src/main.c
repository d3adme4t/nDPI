...
static inline void ndpi_proto_bitmask(struct ndpi_net *n, u_int32_t *var, ndpi_protocol *proto)
{
...
ndpi_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	if(info->t_mark) {
		...
		if(info->proto_id)
			ndpi_proto_bitmask(n,&skb->mark,&proto);
	}
	if(info->t_clsf) {
	        skb->priority = (skb->priority & info->mask) | info->mark;
		if(info->proto_id)
			ndpi_proto_bitmask(n,&skb->priority,&proto);
	}
